...
#[tauri::command]
fn connect(app: AppHandle, server: String, username: String, password: String) -> Result<Session, String> {
...
#[tauri::command]
fn list_games(app: AppHandle) -> Result<Value, String> {
...
#[tauri::command]
fn game_detail(app: AppHandle, slug: String) -> Result<Value, String> {
    let (srv, _) = build_server(&app)?;
...
#[tauri::command]
fn cover_data_url(app: AppHandle, slug: String) -> Result<Option<String>, String> {
        ...
        for entry in entries.flatten() {
            ...
            if p.file_stem().and_then(|s| s.to_str()) == Some(slug.as_str()) {
                if let Ok(bytes) = std::fs::read(&p) {
                    let mime = mime_from_ext(p.extension().and_then(|e| e.to_str()).unwrap_or("jpg"));
...
#[tauri::command]
fn install(app: AppHandle, slug: String) -> Result<install::InstallResult, String> {
    let (srv, cfg) = build_server(&app)?;
    install::install(&app, &srv, &cfg, &slug)
}

#[tauri::command]
fn play(app: AppHandle, slug: String) -> Result<PlayResult, String> {
    let (srv, cfg) = build_server(&app)?;
    let state = config::load_state(&app);
    ...
    let exe = match exe {
        ...
        None => {
            if entry.setup_type == "iso" || entry.setup_type == "installer" {
                return Err("This game was installed by its own setup — launch it from the Start Menu.".into());
            }
    ...
    let workdir = exe_path.parent().unwrap_or(std::path::Path::new(&entry.path)).to_path_buf();
...
#[tauri::command]
fn uninstall(app: AppHandle, slug: String) -> Result<(), String> {
    let (srv, cfg) = build_server(&app)?;
