...
HTTP Basic header (used by the Windows agent and CLI). 401 is returned
*without* WWW-Authenticate so the browser never shows its native popup. With
``auth=none`` everyone acts as the bootstrap admin (single-user mode).
"""
...
def ensure_admin() -> None:
    ...
    Default (no ``LUDEX_AUTH_PASS``): leave the user table empty so the first
    ...
    with SessionLocal() as db:
        if db.scalar(select(func.count()).select_from(User)):
            return
        if s.auth == "none":
            # single-user mode still needs a user row for playtime; login is disabled
